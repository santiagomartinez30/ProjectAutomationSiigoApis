...
public class SeeThatCreateUser {
    ...
    public static Consequence[] correctWith(UserModel userModel) {
        return new Consequence[]{
                ...
                seeThat("name value",
                        response -> LastResponse.received().answeredBy(theActorInTheSpotlight()).getBody().path("name"),
                        equalTo(userModel.getName())),
                seeThat("job value",
                        response -> LastResponse.received().answeredBy(theActorInTheSpotlight()).getBody().path("job"),
